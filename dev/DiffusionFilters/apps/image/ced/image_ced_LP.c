...
/* International Journal of Computer Vision 31, 2/3, 111-127, 1999.         */
/*                                                                          */
...
/** \brief Performs one explicit step of coherence enhancing diffusion 
 for grey value images.
 */
void image_ced_ex_step(
											 ...
											 double rho,     /**< integration scale for structure tensor */
											 double prec,    /**< cutoff precision for convolutions      */
											 double alpha,   /**< CED parameter alpha                    */
											 double C,       /**< CED parameter C                        */
											 ...
											 double **u)     /**< output image                           */
{
	...
	double kappa;     /* anisotropy measure */
	...
	if( bx < 1 || by < 1 )
	{
		printf("image_ced_LP: boundary size too small.\n");
		exit(1);
	}
	...
	for(i=bx; i<nx+bx; i++)
	{
	    ...
	    lambda[1] = alpha;
	    kappa = (lambda[1]-lambda[2])*(lambda[1]-lambda[2]);
	    if( kappa < eps )
	    {
				lambda[2] = alpha;
	    } else {
				lambda[2] = alpha + (1.0-alpha)*exp(-C / kappa);
	    }
...
int main(int argc, char* argv[])
{
	...
	double alpha;           /* parameter for ced */
	double C;               /* parameter for ced */
	...
	if (argc < 6) {
		...
		printf("Usage:\n");
		printf("image_ced_LP [input_image.PGM] [rho] [iters_total] [iters_step] [prefix_out_image (without .PGM)]\n\n");
		exit(0);
	}
  ...
  printf("iters_step = %ld\n", it_step);
	...
	switch(s1) 
	{
		...
		default:
	    fprintf(stderr, "image_ced_LP: scheme must be 0, 1 or 2.");
	    exit(1);
	}
	...
	switch(s2) 
	{
		...
		default:
	    fprintf(stderr, "image_ced_LP: theta must be 0, 1, 2 or 3.");
	    exit(1);
	}
	...
	comment_add(new_comments, "# Coherence enhancing diffusion filtering\n");
	...
	switch(scheme) 
	{
		...
		default:
	    fprintf(stderr, "image_ced_LP: finite difference scheme %d "
							"not known\n", s1);
	    exit(1);
	}
	comment_add(new_comments, "#   integration scale rho: %f\n", rho);
	comment_add(new_comments, "#   cutoff precision:      %f\n", prec);
	comment_add(new_comments, "# CED parameter alpha:     %f\n", alpha);
	comment_add(new_comments, "# CED parameter C:         %f\n", C);
	...
	switch(discr) 
	{
		...
		default:
	    fprintf(stderr, "image_ced_LP: discretisation parameter %d "
							"not known\n", discr);
	    exit(1);
	}
	...
	for(k=1; k<=it; k++)
	{
		image_ced_ex_step(f, nx, ny, bx, by, 1.0, 1.0, sigma, scheme, rho, 
											prec, alpha, C, discr, tau, dt, theta, 
											stencil, u);
