...
`imageData<-` = function (y, value) {
    ...
    y@.Data = value
    ## conversion here should not be possible ! kept for compatibility !
    # line below commented by Kevin Keraudren: it gives unwanted conversion to
    # Truecolor when the image contains only integers
    # if (is.integer(value) && max(value) > 1) y@colormode = TrueColor
    validObject(y)
  } else return(value)
  return (y)
}
