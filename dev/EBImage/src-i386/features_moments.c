...
/* calculates size, mean, var */
/* obj is an Image here and ref is a grayscale one */
SEXP
lib_mean_var (SEXP obj, SEXP ref, SEXP _nobj) { 
  int nprotect, nx, ny, nz, im, i, x, y, nobj, no_objects;
  double * data, * refd, * m, val;
  SEXP res, moments, dm, nm, dmnm;
  ...
  nprotect++;
  SET_STRING_ELT( nm, 0, mkChar("size") );
  SET_STRING_ELT( nm, 1, mkChar("mean") );
  SET_STRING_ELT( nm, 2, mkChar("var") );
  SET_VECTOR_ELT( dmnm, 1, nm );
  UNPROTECT( 1 ); nprotect--; // nm
  ...
  for ( im = 0; im < nz; im++ ) {
    ...
    /* create result storage */
    SET_VECTOR_ELT( res, im, (moments = allocVector(REALSXP, 3 * nobj)) );
    /* reset result */
    m = REAL( moments );
    for ( i = 0; i < 3 * nobj; i++ ) m[i] = 0.0;
    /* set dim, etc */
    PROTECT( dm = allocVector(INTSXP, 2) );
    nprotect++;
    INTEGER( dm )[0] = nobj;
    ...
    for ( x = 0; x < nx; x++ )
      for ( y = 0; y < ny; y++ ) {
        ...
        if ( ref != R_NilValue )
          ...
          val = 1.0;
        m[i           ] += 1.0;       /* size in pixels */
        m[i +     nobj] += val;       /* mean intensity */
        m[i + 2 * nobj] += val * val;   /* variance */
      }
    
    /* convert M10, M01 to xm and ym */
    for ( i = 0; i < nobj; i++ ) {
      if ( (val = m[i]) == 0.0 ) continue;
      m[i +     nobj] /= val;
      m[i + 2 * nobj] /= val;
      m[i + 2 * nobj] -= m[i + nobj] * m[i + nobj]; // E[X^2] - (E[X])^2
    }
  }
